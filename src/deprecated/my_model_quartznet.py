...
class MyModel(ASRModel, ExportableEncDecModel, ASRModuleMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        with open_dict(self._cfg):
            ...
            if hasattr(self._cfg, 'corruption_prob'):
                self.corruption_prob = self._cfg.corruption_prob
...
#             "pitch_predicted": NeuralType(('B', 'T_text'), RegressionValuesType()),
    ...
    @typecheck()
    def forward(
        self, input_signal=None, input_signal_length=None, processed_signal=None, processed_signal_length=None, dae=False
    ):
        """
        ...
        for i in range(processed_signal.shape[0]):
            for j in range(processed_signal.shape[1]):
                if random.random() < self.corruption_prob:
                    processed_signal[i][j] = 0
    ...
    def training_step(self, batch, batch_nb):
        ...
        batch = [torch.vstack((batch[0], batch[0])),
                torch.hstack((batch[1], batch[1])),
                torch.vstack((batch[2], batch[2])),
                torch.hstack((batch[3], batch[3]))]
            
        for i in range(len(batch)):
            try:
                for j in range(len(batch[i])// 2, len(batch[i])):
